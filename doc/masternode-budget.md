...
SkyHub now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<skyhub_address\> \<monthly_payment_skyhub\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ SkyHub, _12_ times over the course of a year totaling _24000_ SkyHub.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<skyhub_address\> \<monthly_payment_skyhub\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` SkyHub.
 ...
 * vote-many          - Vote on a SkyHub initiative
 * vote-alias         - Vote on a SkyHub initiative
 * vote               - Vote on a SkyHub initiative/budget
