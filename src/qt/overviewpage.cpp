...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SHB)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("SHB")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSkyHubAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSkyHubAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSkyHubAmount = strAnonymizeSkyHubAmount.remove(strAnonymizeSkyHubAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkyHubAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSkyHubAmount * COIN) nMaxToAnonymize = nAnonymizeSkyHubAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSkyHubAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSkyHubAmount));
        strAnonymizeSkyHubAmount = strAnonymizeSkyHubAmount.remove(strAnonymizeSkyHubAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkyHubAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
